...
func createTables() error {
	...
	// Migration: update existing briefing prompt to viewpoint format
	// This ensures existing DBs (which already have the old prompt) get the new version
	newBriefingPrompt := `根据以下文章，生成一份"观点提炼"简报。不是摘要文章讲什么，而是提炼每篇文章的主张/立场/观点。
...
{articles}`
	newBriefingSystem := `你是科技新闻分析师，负责提炼文章的核心观点和立场。输出必须是中文 JSON，严格按格式来，不要有任何额外解释。`
