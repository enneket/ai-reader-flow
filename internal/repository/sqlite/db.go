...
{content}`,
		...
		},
		{
			promptType: "briefing",
			name:       "简报生成",
			prompt: `根据以下文章，生成一份主题分组简报。

任务要求：
1. 将文章按主题/话题归类（内容相近的归为一组）
2. 每个主题起一个简短有力的名字（几个字到十几个字，越精炼越好）
3. 每个主题写一段介绍，像在跟朋友聊天分享那样自然、有趣、有信息量
4. 每篇文章写一句 insight，说明这篇文章有意思在哪里、为什么值得看

写作风格要求：
- 像在社交媒体上跟朋友分享那样自然，不要端着
- 不要用"核心发现是..."、"相比传统..."、"对...有参考价值"、"值得关注"这类模板表达
- 有话则长、无话则短，不要凑字数
...
输出格式（严格 JSON，不要有其他内容）：
{
  "topics": [
    {
      "name": "主题名称",
      "summary": "一段自然的话，像在跟朋友介绍这个主题在聊什么、为什么有意思",
      "articles": [
        {"id": 1, "insight": "一句话描述这篇文章有意思的地方，像在跟朋友说"}
      ]
    }
  ]
}
...
{articles}`,
			system:    `你是一个科技新闻爱好者，擅长发现有趣的内容并用轻松自然的方式分享。输出必须是中文 JSON，严格按格式来，不要有任何额外解释。`,
			maxTokens: 16384,
			isDefault: 1,
		},
		{
